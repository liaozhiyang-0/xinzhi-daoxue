"""Prepare the legacy request envelope around a durable Runtime launch.

This service owns the compatibility-only work that must happen before a
Runtime or legacy execution can start: conversation context assembly, bounded
overall-route refinement, and the immutable legacy execution plan. It does not
invoke a Provider and does not mutate the Task row; callers apply the returned
route decision to their own Task/SSE transaction.
"""
...
class RuntimeCompatibilityPreparation:
    """Result of compatibility preparation, ready for Runtime launch."""
...
class RuntimeCompatibilityPreparationService:
    """Keep legacy route/context preparation outside the Runtime executor."""
    ...
    async def prepare(
        ...
        *,
        ...
        if self.context_assembly is not None and not runtime_resume:
            conversation_bundle = await self.context_assembly.assemble(
                ...
                course_id=course_id,
                task_family=self._route_task_family(
                    decision, fallback_task_family
                ),
                agent_id=agent_id,
            )
            request = self.with_conversation_context(
        ...
        if self.overall_router is not None and not runtime_resume:
            ...
            if outcome.used:
                ...
                if self.context_assembly is not None:
                    ...
                    conversation_bundle = (
                        await self.context_assembly.assemble(
                            db,
                            session_id=session_id,
                            user_id=user_id,
                            current_message_id=current_message_id,
        ...
        return RuntimeCompatibilityPreparation(
            request=request,
            decision=decision,
            agent_id=final_agent_id,
