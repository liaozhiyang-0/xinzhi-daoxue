...
export function createTaskTransport({ api, ownedTaskUrl, state, addMessage, selectContextTab, liveProgressData, updateLiveProgress, refreshRuntimeTaskControls, renderLongWaitNotice, }) {
    async function waitForTask(id, runSequence) {
        ...
        return new Promise((resolve, reject) => {
            ...
            longWaitTimer = window.setInterval(() => {
                if (!settled)
                    renderLongWaitNotice(performance.now() - waitStartedAt);
            }, 5000);
            ...
            events.onerror = () => {
                if (settled || reconnectPollTimer !== null)
                    return;
