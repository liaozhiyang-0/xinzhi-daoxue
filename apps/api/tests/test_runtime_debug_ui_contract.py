...
def test_execution_console_exposes_agent_runtime_operator_surfaces() -> None:
    ...
    assert "runtimeTimingSurface" in script
...
def test_execution_sse_reconnect_preserves_existing_loader() -> (
    ...
    assert (
        "executionEventStreamRetryAttempts >= executionEventStreamRetryDelaysMs.length"
        in script
    )
