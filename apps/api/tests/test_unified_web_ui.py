...
def test_markdown_renderer_uses_text_nodes_not_untrusted_html(client) -> None:
    ...
    assert "function resetConversation()" in student
    assert "clearImage();" in student
    assert "latexStructureSafe" in script
    assert "repairTrailingLatexGroups" in script
    assert "normalizeLatexDelimiters" in script
    assert '"}".repeat(braceDepth)' in script
    assert 'let normalized = ""' in script
    assert r'.replace(/\\math\b/gu, "")' in script
    assert r'.replace(/[_^]\s*\{\s*$/u, "")' in script
    assert r"\d+\.\s+" in script
    assert "already-delimited inline formulas" in script
...
def test_workspace_shows_six_showcase_examples(client) -> None:
    ...
    assert (
        "const requestedCourse = learningFollowUp?.course_id || "
        'state.activeCourse || "AUTO";'
        in script
    )
    assert (
        "const requestedIntent = learningFollowUp?.intent || "
...
def test_workspace_showcase_wiring_preserves_card_intent_and_course() -> None:
    root = Path(__file__).resolve().parents[3]
    script = (root / "apps/api/app/static/debug/workspace.js").read_text(
        encoding="utf-8"
    )

    assert 'state.activeCourse = button.dataset.course || "AUTO";' in script
    assert 'state.intentOverride = button.dataset.intent || "";' in script
    edit_start = script.index('if (state.activeScenarioPrompt')
    edit_end = script.index(
        'autoGrow(); updateAutoDetection(); updateResearchAnalysisPanel();',
        edit_start,
    )
    assert 'state.activeCourse = "AUTO";' in script[edit_start:edit_end]
