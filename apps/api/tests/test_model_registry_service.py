...
def test_registry_loads_models_and_routes() -> None:
    ...
    assert registry.get_route("academic_writing").primary == "qwen_vision_primary"
    assert registry.get_route("academic_writing").fallback == "qwen_text_fast"
...
@pytest.mark.asyncio
async def test_runtime_opt_in_allows_structured_output_fallback() -> None:
    spark = FakeProvider(
        "iflytek_spark",
        "spark-x",
        [
