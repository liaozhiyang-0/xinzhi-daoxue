...
## 140. 2026-08-12 real Edge session persistence and cancellation recovery fix

- A registered student account was created through the formal Edge registration flow. Login, completed-task refresh, and course evidence restoration all succeeded; the restored answer retained its evidence chip, inline citation buttons, and read-only source dialog.
- The official /workspace auto-dispatch path completed a Circuit Theory question with three course evidence items and a safe fallback warning. A prior approval gate observed on a ?intent=lesson_prep URL was not counted as a homepage routing defect because that URL intentionally constrained the task mode.
