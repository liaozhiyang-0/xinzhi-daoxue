...
Web / API client
  ...
  -> existing TaskCreationService / TaskRunner / SSE
       ...
       -> professional tools
       -> Local Runtime handlers / model Provider
...
多图和 PDF 不会进入外部工作流请求：Supervisor 将它们标记为需要本地预处理，并选择安全本地 Runtime 路径。

`/chat` 对 CT/AE/DE 学习类请求优先选择本地 RAG Agent；学生 Workspace 的 `/tasks` 继续使用同一 TaskRunner，并将备课、作业初审、学术写作和数据分析 Agent 接入本地 Runtime。备课使用同一任务内构建的 RAG 上下文，作业初审保持 reference-only。正式学生前端不显示 Provider 内部实现名或运行时元数据。
