...
1. 保持 `.env`、Flow ID 和密钥不变，不在会议现场编辑。
...
5. 默认 Preflight 不消耗云端额度；只有明确需要真实探测时才加 `--with-cloud`。

页面地址：`http://127.0.0.1:8000/`、`/student`、`/demo?presentation=1`。云端 LEARN/SOLVER 通常需要约 20 秒，演示时应提前说明等待来自云端而非本地页面。
...
7. 在演示中心载入“稳定降级”，展示云端不可用时的本地回答和免责声明。
...
- 云端超时：切换演示中心“稳定降级”场景，不修改 Flow ID。
- 图片失败：使用清晰 PNG/JPEG/WebP 且小于 8MB；也可只展示上传预览。
- 状态接口失败：系统页仍会打开并标记无法获取。

演示结束后，使用启动脚本打印的 PID 执行 `Stop-Process -Id <PID>`；不要结束非本次启动的共享 Qdrant。不要在现场打印 `.env`、完整 Trace、Authorization 或 Flow ID。
