...
const portNumber = parseBrowserPort(port, "auth browser acceptance");
...
async function waitForServer() {
  await assertPortAvailable(portNumber, "auth browser acceptance");
  ...
  await waitForSpawnedHealth({ server, baseURL, label: "auth browser acceptance", attempts: 60 });
}
